...
import { ClientProps, MaterialCategory, MaterialType, insertMaterialProps, insertedColumns } from './utils'
...
const CreateEvent = () => {
  ...
  const insertMaterial = (event: React.FormEvent, materialName: string, quantity: number, index: number, price: number) => {
    ...
    const newMaterialInsert: insertMaterialProps = { name: materialName, quantity, key: index.toString(), price }
  ...
  const totalAmount = insertedMaterial.reduce((sum, material) => sum + (material.price * material.quantity), 0);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: any, index: number) => (
        <LucideTrash2 onClick={() => removeMaterial(index)} className='h-5 w-5 inline-block text-primary ml-2  cursor-pointer' />
      ),
    },
  ];
      ...
      <div className='h-full bg-primary'>
        ...
        <form action="" className='flex flex-col  w-[800px] rounded-xl bg-opacity-30 bg-slate-400 p-16 mx-auto'>
          ...
          <div className='py-8'>
            ...
            <Table scroll={{y: 500}} dataSource={insertedMaterial} columns={columns} pagination={false} rowKey="key" />
