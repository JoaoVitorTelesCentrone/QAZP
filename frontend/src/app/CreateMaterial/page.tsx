...
const CreateMaterial = () => {
  ...
  const redirectM = () => { 
    ...
    const createMaterialRequest = async () => { 
      ...
      console.log(data)
      try {
        const response = await axios.post('http://localhost:5196/api/Material', data)
    ...
    <div>
      <UserSideMenu />
      <Toaster richColors />
        <Link href='/Materials'><ArrowLeft className='my-12 mx-24' /></Link>
      ...
      <div className='flex flex-col border-2 rounded-xl bg-primary bg-opacity-20 p-8 w-[600px] mx-auto justify-around'>
            <div className="flex flex-col">
                ...
                <Input onChange={(e) => setName(e.target.value)} placeholder='Nome' className='border-2  bg-white placeholder:text-primary mx-4 my-1 w-[90%]' />
            </div>
        <div className="flex w-full my-2 mx-4">
                ...
                <DropdownMenu >            
                    ...
                    <DropdownMenuContent className='border-2 p-4 bg-white my-1 rounded-xl'>
                          {MaterialCategory.map((category, index) => (
                            <>
                              <DropdownMenuItem className='cursor-pointer my-1' onClick={() => getCategoryNameAndIndex(category.name, category.index)} key={index}>{category.name}</DropdownMenuItem>
            ...
            <div className='w[50%]'>
              ...
              <Input type='number' onChange={(e) => setPrice(e.target.valueAsNumber)} placeholder='Digite o preço' className='border-2  bg-white placeholder:text-primary' />
            </div>
        </div>
            <Button onClick={() => createMaterialRequest()} className='text-white w-[120px] my-4 ml-96 p-2'>Criar material</Button>
      </div>
    </div>
  )
}
